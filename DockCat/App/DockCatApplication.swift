...
final class DockCatApplication: NSObject, NSApplicationDelegate {
    ...
    @objc private func taskManager() {
        Task {
            await MainActor.run {
                TaskReminderManager.shared.showTaskManagerWindow()
            }
        }
    }
}
