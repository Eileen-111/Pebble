...
final class AssetPackLoader {
    ...
    private func copyDefaultPackIfNeeded(to customRoot: URL) {
        let destination = customRoot.appendingPathComponent("default-pet", isDirectory: true)
        guard !fileManager.fileExists(atPath: destination.path) else {
            return
        }
        guard let source = defaultPackCandidates().first(where: { isDirectory($0) }) else {
            DockCatLog.assets.error("Failed to seed default-pet because bundled DefaultCat was not found")
            return
        }
        do {
            try fileManager.copyItem(at: source, to: destination)
        } catch {
            DockCatLog.assets.error("Failed to seed default-pet: \(error.localizedDescription)")
        }
    }

    private func seedDefaultPackIconsIfNeeded(in customRoot: URL) {
        let defaultPackRoot = customRoot.appendingPathComponent("default-pet", isDirectory: true)
        ...
        do {
            ...
            DockCatLog.assets.error("Failed to seed default-pet app icons: \(error.localizedDescription)")
        }
    ...
    private func repairDefaultPackManifestIconsIfNeeded(at root: URL) {
        ...
        guard fileManager.fileExists(atPath: manifestURL.path),
              ...
              isDefaultPetManifest(manifest),
              manifest.appIcons == nil
        else {
            return
        }
        do {
            try defaultPetManifestData(withAppIcons: true).write(to: manifestURL, options: .atomic)
        } catch {
            DockCatLog.assets.error("Failed to repair default-pet manifest app icons: \(error.localizedDescription)")
        }
    }
    ...
    private func isUnmodifiedTemplateManifest(_ manifest: AssetManifest) -> Bool {
        manifest.id == "my-cat"
            ...
            && manifest.canvasWidth == 650
            && manifest.canvasHeight == 650
    ...
    private func isDefaultPetManifest(_ manifest: AssetManifest) -> Bool {
        manifest.id == "default-pet"
            && manifest.name == "E"
            && manifest.author == "Eileen"
            && manifest.canvasWidth == 650
            && manifest.canvasHeight == 650
            && manifest.defaultAnchor == .init(x: 0.5, y: 0.88)
            && manifest.poses == .init(
    ...
    private func defaultPetManifestData(withAppIcons: Bool) throws -> Data {
